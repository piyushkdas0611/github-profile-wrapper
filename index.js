document.addEventListener("DOMContentLoaded", () => {
  ...
  showDetails.addEventListener("submit", async () => {
  ...
  function showProfile(data) {
    ...
    profileInfoDiv.innerHTML = `<div class="card">
        ...
        <div class="card-body">
            ...
            <div class="card-subHeading"> User Name : ${data.login}</div>
            <div class="card-text">
                <p>${data.bio}</p>
  ...
  async function showReposInfo(userName) {
    ...
    function renderProjects(sortedProjects) {
      ...
      for (let i = 0; i < sortedProjects.length; i++) {
        reposInfoDiv.innerHTML += `<div class="repo">
                <div class="repo-body">
                    <div class="repo-title">${sortedProjects[i].name}</div>
                    ${
                      sortedProjects[i].description == null
                        ? '<div class="repo-description">No Description</div>'
                        : `<div class="repo-description">${sortedProjects[i].description}</div>`
                    }
                    <div class="repo-subHeading"> <span>${
